...
R_O(const int func) {
    ...
    switch (func) {
        ...
        case f_form:
            if ((context->rexx_proc)[(context->rexx_rx_proc)].form ==
                SCIENTIFIC)
...
R_oSoS() {
    ...
    LZEROSTR(*ARGR);
    if (ARG1 == NULL)
        RxReadVarTree(ARGR, (context->rexx_proc)[(context->rexx_rx_proc)].scope,
                      NULL, option);
    else {
        ...
        LASCIIZ(str);
        leaf = RxVarFindName(
                (context->rexx_proc)[(context->rexx_rx_proc)].scope, &str,
                &found);
        if (found == 0) return;
...
R_sourceline() {
    ...
    if (l == 0) { /* count the lines of the program */
        ...
        while (rxf == (context->compileCompileClause)[i].fptr
               && (context->compileCompileClause)[i + 1].line >=
                  (context->compileCompileClause)[i].line) {
            i++;
        ...
        if (l > 1) {
            ...
            for (i = 0; rxf == (context->compileCompileClause)[i].fptr
                        && (context->compileCompileClause)[i + 1].line >=
                           (context->compileCompileClause)[i].line; i++) {
                if ((context->compileCompileClause)[i].line == l) {
                    ...
                    c++;
                    goto linefound;
                } else if ((context->compileCompileClause)[i].line > l) {
                    if (i > 0) {
                        i--;
                        c = (context->compileCompileClause)[i].ptr;
