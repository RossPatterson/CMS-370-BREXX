...
RxInitVariables(void) {
    ...
    LINITSTR((context->variable_varidx));
    Lfx(&(context->variable_varidx), 250);
...
RxDoneVariables(void) {
    ...
    BinDisposeLeaf(&(context->variable_PoolTree),
                   (context->variable_PoolTree).parent, FREE);
} /* RxDoneVariables */
...
RxVarFree(void *var) {
    ...
    if (v->exposed == NO_PROC) {
        ...
        LFREESTR(v->value);
        FREE(var);
    } else if (v->exposed == (context->rexx_rx_proc) - 1)
        v->exposed = NO_PROC;
} /* RxVarFree */
...
RxVarAdd(Scope scope, PLstr name, int hasdot, PBinLeaf stemleaf) {
    ...
    if (hasdot == 0) {
        ...
        if (stemleaf == NULL) { /* we must create the stem */
            ...
            LASCIIZ(aux);
            tree = scope + (context->variable_hashchar)[(byte) LSTR(
                    *(context->variable_varname))[0]];
...
RxVarFind(const Scope scope, const PBinLeaf litleaf, bool *found) {
    ...
    if (!inf->stem) {  /* simple variable */
        ...
        *found = (leaf != NULL);
        ...
        if ((context->interpreRx_id) != NO_CACHE &&
...
**/
            leaf = tree->parent;
            while (leaf != NULL) {
                cmp = STRCMP(LSTR(*(context->variable_varname)),
                             LSTR(leaf->key));
                if (cmp < 0)
                    leaf = leaf->left;
                else if (cmp > 0)
        ...
        for (i = 1; i < inf->stem; i++) {
            if (i != 1) {
                /* append a dot '.' */
                LSTR((context->variable_varidx))[LLEN(
                        (context->variable_varidx))] = '.';
            ...
            if (infidx == NULL) {
                ...
                if (LMAXLEN((context->variable_varidx)) <= l)
                    Lfx(&(context->variable_varidx), l);
                MEMCPY(LSTR((context->variable_varidx)) +
                       LLEN((context->variable_varidx)), LSTR(*aux),
                       LLEN(*aux));
                LLEN((context->variable_varidx)) = l;
            } else if ((context->interpreRx_id) != NO_CACHE &&
                       infidx->id == (context->interpreRx_id)) {
                register PLstr lptr;
                ...
                if (LMAXLEN((context->variable_varidx)) <= l)
                    Lfx(&(context->variable_varidx), l);
                MEMCPY(LSTR((context->variable_varidx)) +
                       LLEN((context->variable_varidx)), LSTR(*lptr),
                       LLEN(*lptr));
                LLEN((context->variable_varidx)) = l;
            } else {
                /* search for aux-variable */
                tree = curscope +
                       (context->variable_hashchar)[(byte) LSTR(*aux)[0]];
        ...
        if ((context->interpre__trace))
            if ((context->rexx_proc)[(context->rexx_rx_proc)].trace ==
                ...
                int i;
                FPUTS("       >C>  ", STDERR);
                for (i = 0; i < (context->rexx_nesting); i++)
                    FPUTC(' ', STDERR);
                FPUTC('\"', STDERR);
        ...
        if (leaf == NULL) {
            *found = FALSE;
            Lstrcpy(&(context->variable_stemvaluenotfound),
                    (context->variable_varname));
            Lstrcat(&(context->variable_stemvaluenotfound),
                    &(context->variable_varidx));
            return NULL;
        }
...
RxVarExpose(Scope scope, PBinLeaf litleaf) {
    ...
    if (inf->stem == 0) {
        ...
        tree = stemvar->stem + (context->variable_hashchar)[(byte) LSTR(
                (context->variable_varidx))[0]];
        return BinAdd(tree, &aux, var);
    }
} /* RxVarExpose */
...
RxSetSpecialVar(int rcsigl, long num) {
    ...
    Lfx(&value, 0);
    Licpy(&value, num);
    RxVarSet((context->rexx_proc)[(context->rexx_rx_proc)].scope, varleaf,
             &value);
    LFREESTR(value);
} /* RxSetSpecialVar */
...
RxScopeMalloc(void) {
    ...
    for (i = 0; i < VARTREES; i++) BINTREEINIT(scope[i]);
    return scope;
} /* RxScopeMalloc */
