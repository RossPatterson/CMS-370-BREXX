...
nextchar(int instring) {
    ...
    if (!instring && *(context->nextsymbsymbolptr) == '/' &&
        *((context->nextsymbsymbolptr) + 1) == '*') {
        /* search for comment */
        ...
        for (;;) {
            ...
            (context->nextsymbsymbolptr)++;
            if ((*((context->nextsymbsymbolptr) - 1) == '/') &&
                (*(context->nextsymbsymbolptr) == '*')) {
                level++;
                (context->nextsymbsymbolptr)++;
            } else if ((*((context->nextsymbsymbolptr) - 1) == '*') &&
                       (*(context->nextsymbsymbolptr) == '/')) {
                --level;
    ...
    switch ((context->lstring_l2u)[(byte) *(context->nextsymbsymbolptr)]) {
        ...
        case '/': NEXTCHAR;
            if (*(context->nextsymbsymbolptr) == '/') {
                ...
                NEXTCHAR;
            } else if (*(context->nextsymbsymbolptr) == '=') {
                NEXTCHAR;
                if (*(context->nextsymbsymbolptr) == '=') {
        ...
        case 0:
            ...
            else if ((context->nextsymbsymbolstat) == in_if_init_st)
                (context->lstring_Lerror)(ERR_THEN_EXPECTED, 0);
            else if ((context->nextsymbsymbolstat) != normal_st)
                (context->lstring_Lerror)(ERR_SYMBOL_EXPECTED, 0);
...
identifier(int isnumber) {
    ...
    if ((context->nextsymbsymbolstat) == in_do_init_st) {
        if (!Lcmp(&(context->nextsymbsymbolstr), "BY"))
            (context->nextsymbsymbol) = by_sy;
        else if (!Lcmp(&(context->nextsymbsymbolstr), "FOR"))
            (context->nextsymbsymbol) = for_sy;
        else if (!Lcmp(&(context->nextsymbsymbolstr), "TO"))
            (context->nextsymbsymbol) = to_sy;
        else if (!Lcmp(&(context->nextsymbsymbolstr), "UNTIL"))
            (context->nextsymbsymbol) = until_sy;
        else if (!Lcmp(&(context->nextsymbsymbolstr), "WHILE"))
            (context->nextsymbsymbol) = while_sy;
    } else if ((context->nextsymbsymbolstat) == in_parse_value_st) {
        if (!Lcmp(&(context->nextsymbsymbolstr), "WITH"))
            (context->nextsymbsymbol) = with_sy;
    } else if ((context->nextsymbsymbolstat) == in_if_init_st) {
        if (!Lcmp(&(context->nextsymbsymbolstr), "THEN"))
            (context->nextsymbsymbol) = then_sy;
    }
} /* identifier */
        ...
        if (*(context->nextsymbsymbolptr) == quote) {
            ...
            if (*(context->nextsymbsymbolptr) == '(') {
                ...
                *s = '\0';
                LLEN((context->nextsymbsymbolstr)) = l;
                return;
            } else if (STRCHR("bBxXhH", *(context->nextsymbsymbolptr))) {
                /* check next char */
                char nc = (context->lstring_l2u)[(byte) *(
                        (context->nextsymbsymbolptr) + 1)];
                ...
                *s = '\0';
                ...
                LFREESTR(A);
                return;
            } else if (*(context->nextsymbsymbolptr) == quote) {
                *s++ = *(context->nextsymbsymbolptr);
                l++;
            } else {
                *s = '\0';
            ...
            *s++ = ' ';
            l++;
        } else if (*(context->nextsymbsymbolptr) == 0)
